...
import { GetAdminProducts, useDelete, useUpdateIsPublish } from '@/lib/utils/admin/adminQueryFun'
...
export default function AdminProducts() {
  ...
  const [productSelected, setProductSelected] = useState<productTypes | null>(null)
  const [filterValue, setFilterValue] = useState('All Products')
  const [filterOpen, setFilterOpen] = useState(false)
  const [sortBy, setSortBy] = useState('createdAt_desc')
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false)
  const [productToDelete, setProductToDelete] = useState<productTypes | null>(null)
  ...
  return (
    <div className="space-y-6">
      ...
      {sortedProducts.length === 0 ? (
        <div className="text-center text-muted-foreground">No products found. Try adding one.</div>
      ) : (
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {sortedProducts.map((p) => (
            ...
            >
              <div className="relative h-48">
                <Image src={p.thumbnailUrl as string} alt={p.title} fill className="object-cover" />
                <Badge
