...
export async function GET(request: NextRequest) {
  ...
  if (!assetId || !token || !payerId) {
    return NextResponse.redirect(new URL(`/gallery/${assetId}?missinParams=true`, request.url))
  }
  ...
  try {
    ...
    if (data.status === 'COMPLETED') {
      ...
      return NextResponse.redirect(new URL(`/products/${assetId}?success=true `, request.url))
    } else {
      return NextResponse.redirect(
    ...
    console.log(error)
    return NextResponse.redirect(new URL(`/products/${assetId}?error=server_error`, request.url))
  }
}
