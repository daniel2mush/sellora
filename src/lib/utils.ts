...
export function convertOklchToRgb(element: HTMLElement) {
  ...
  for (const el of elements) {
    ...
    if (computedStyles.color.includes("oklch")) {
      ...
      try {
        ...
        console.warn(
          `Failed to parse oklch color on ${el.tagName}: ${oklchColor}`,
          error
        );
    ...
    if (computedStyles.backgroundColor.includes("oklch")) {
      ...
      try {
        ...
        console.warn(
          `Failed to parse oklch background on ${el.tagName}: ${oklchColor}`,
          error
        );
  ...
  const remainingOklch = elements.some((el) => {
    ...
    if (hasOklch) {
      console.warn(
        `Remaining oklch in ${el.tagName} (class: ${el.className || "none"})`,
        {
          color: styles.color,
  ...
  if (remainingOklch) {
    console.warn(
      "Warning: Some oklch colors remain after conversion. Check computed styles."
    );
  } else {
